...
HWY_NOINLINE void TestAllMedian() { TestMedian3<LaneTraits<OrderAscending>>(); }
...
static HWY_NOINLINE void TestBaseCaseAscDesc() {
  ...
  auto aligned_keys = hwy::AllocateAligned<T>(N + base_case_num);
  auto buf = hwy::AllocateAligned<T>(base_case_num + N);

  const size_t N1 = st.LanesPerKey();
  for (bool asc : {false, true}) {
    for (size_t len = HWY_MAX(1, N1); len <= base_case_num; len += N1) {
      for (size_t misalign = 0; misalign < N; ++misalign) {
        ...
        HWY_ASSERT(VerifySort(st, input_stats, keys, len));
        for (size_t i = 0; i < misalign; ++i) {
...
static HWY_NOINLINE void TestBaseCase01() {
  ...
  constexpr int kDebug = 0;
  auto keys = hwy::AllocateAligned<T>(N + base_case_num);
  auto buf = hwy::AllocateAligned<T>(base_case_num + N);

  const size_t N1 = st.LanesPerKey();
  for (size_t len = HWY_MAX(1, N1); len <= HWY_MIN(base_case_num, 16);
       len += N1) {
    if (kDebug) {
      printf("============%s 01 N1 %zu len %zu\n",
             hwy::TypeName(T(), 1).c_str(), N1, len);
    }
    for (uint64_t bits = 0; bits < ((1ull << len) - 1); ++bits) {
      InputStats<T> input_stats;
      for (size_t i = 0; i < len; ++i) {
        keys[i] = (bits & (1ull << i)) ? 1 : 0;
        input_stats.Notify(keys[i]);
        if (kDebug >= 2) printf("%3zu: %f\n", i, double(keys[i]));
      }
      for (size_t i = len; i < base_case_num + N; ++i) {
      ...
      HWY_ASSERT(VerifySort(st, input_stats, keys.get(), len));
      for (size_t i = len; i < base_case_num + N; ++i) {
        if (keys[i] != hwy::LowestValue<T>())
          HWY_ABORT("Overrun right at %zu\n", i);
      }
 ...
 public:
  ...
  bool Verify(const Dist dist, const T* output) {
    ...
    for (size_t i = 0; i < copy_.size(); ++i) {
      if (copy_[i] != output[i]) {
        printf("Asc %d mismatch at %zu: %f %f\n", Order().IsAscending(), i,
               double(copy_[i]), double(output[i]));
        return false;
      }
    }
    return true;
  ...
  for (Algo algo : AlgoForTest()) {
    ...
    for (Dist dist : AllDist()) {
      for (size_t misalign : {size_t{0}, size_t{st.LanesPerKey()},
        ...
        HWY_ASSERT(VerifySort(st, input_stats, keys, num));
...
void TestAllSort() {
  ...
  // Unavailable - these take too long to build on MSVC.
...
#else
void TestAllMedian() {}
void TestAllBaseCase() {}
void TestAllPartition() {}
void TestAllGenerator() {}
void TestAllSort() {}
#endif
...
// TODO(janwas): remove once SVE is tested
#if HWY_TARGETS & (HWY_SVE | HWY_RVV)
  fprintf(stderr, "Skipping SVE sort_test\n");
  return 0;
#endif
