...
struct Key128 {
  ...
  // Only called for 4 keys, so same as a full Reverse.
  template <class D>
  ...
  // This is only called for 4-col networks, i.e. 512-bit vectors.
  template <class D>
...
class Traits128 : public Base {
 ...
 public:
  ...
  HWY_INLINE Vec<D> SortPairsDistance4(D, Vec<D>) const {
    HWY_ASSERT(0);  // Only used in Merge16, which requires 2048 bit vectors
  }
};
