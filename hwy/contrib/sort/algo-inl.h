...
Vec<DU64> RandomValues(DU64 du64, Vec<DU64>& s0, Vec<DU64>& s1,
  ...
  const Repartition<T, DU64> d;
  // Convert value to floating-point and return its bit representation.
  // First cast to signed to enable the conversion.
  const Repartition<MakeSigned<T>, DU64> di;
  return BitCast(du64, ConvertTo(d, BitCast(di, values)));
}
