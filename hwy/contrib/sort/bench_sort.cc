...
HWY_NOINLINE void BenchPartition() {
  ...
  for (size_t log2 = 24; log2 < 25; ++log2) {
    // for (size_t log2 = 12; log2 < 28; ++log2) {
...
HWY_NOINLINE void BenchBase(std::vector<Result>& results) {
  ...
  constexpr size_t kMul = 600;  // ensures long enough to measure
  ...
  for (size_t rep = 0; rep < kReps; ++rep) {
    ...
    HWY_ASSERT(VerifySort(st, input_stats, keys.get(), num));
  }
  HWY_ASSERT(sum < 1E99);
  ...
  for (Algo algo : AlgoForBench()) {
    for (Dist dist : AllDist()) {
      ...
      for (size_t rep = 0; rep < kReps; ++rep) {
        ...
        HWY_ASSERT(VerifySort(st, input_stats, aligned.get(), num));
      }
